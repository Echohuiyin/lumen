...
7. 远端规格文件（由 `${LUMEN_PROJECT_ROOT}` 指定）是规格事实源；每次更新后同步本地副本并校验 SHA-256 一致。
8. 所有 prompt 必须明确 Linux 内核维护定位，使用诊断性语言，不得把工作描述为漏洞发现或利用。
9. 禁止硬编码开发者目录、服务器地址、凭据、架构/案例专属路径或本机工具链位置。运行时路径、架构、镜像、源码和服务端点必须来自输入制品、项目配置、环境变量或命令行参数；默认值只能是与部署位置无关的相对路径或可检测能力。新增部署入口必须在不同用户和不同 checkout 路径下通过可移植性测试。

## 4.1 Kernel Expert 的 Codex 与无 fallback 约束

Kernel Expert 必须使用项目隔离的 Codex CLI，并且只加载仓库维护的 skills。该角色不允许切换到 Claude、OpenCode、Anthropic/OpenAI 直连 backend、其他 provider 或另一个 model/profile。

1. `backend` 必须是 `codex`，调用入口必须是非交互式 `codex exec --json --ephemeral`。
2. 每次调用必须使用项目内独立的 HOME/CODEX_HOME，忽略用户 config/rules；skills 只能从 `.agents/skills` 加载，并禁用 plugin、app 和 multi-agent 扩展。
3. 认证只允许来自 mode `0600` 的隔离 `auth.json`，或本次调用作用域内的 `CODEX_API_KEY`；认证文件不得进入版本库或日志。
4. Semcode MCP 必须声明为 `required`。缺少 Semcode 可执行文件、索引、项目 skills 或认证时必须返回 `blocked`，禁止退回 grep、另一个工具、账号、provider、backend 或 model。
5. Codex 仅可写当前 session 工作目录。目标内核源码保持只读，不得通过额外 writable directory 暴露给 Codex。
6. 不允许配置 `model_pool`、账号列表或 quota failover。Codex quota、billing、认证、超时或 CLI 启动失败时，Kernel Expert 必须保留真实 JSONL/错误证据并返回显式 `blocked`；`429`、`402`、`Insufficient Balance` 等错误归类为 `QUOTA_EXHAUSTED` 或对应错误码。
7. Kernel Expert 只生成基于源码/log 的用户态 C reproducer contract；Test Expert 才能在 QEMU 内编译、执行并判断调用链是否一致。

## 4.2 Rootfs 与 SSH 身份绑定

Test Expert 使用 `input.txt`/`KernelExpertContract.rootfs_path` 声明的 rootfs。它不会为每个案例重新编译 rootfs，而是在每个 try-out 创建独立的可写稀疏副本。显式声明的 rootfs 必须与同目录下的 `lumen_qemu_ed25519` 配套；缺少配套 key 时立即 `BLOCKED_BASE_IMAGE_MISSING`，禁止退回到不匹配的部署默认 key。镜像源保持只读，实际镜像副本和使用的 key 路径必须写入 `test_attempt_contract.artifacts`。
...
## Deployment portability constraint

All implementation, configuration, and deployment paths must remain portable
across developers and supported hosts. Do not add developer-specific absolute
paths, server addresses, credentials, fixed case names, or fixed function
