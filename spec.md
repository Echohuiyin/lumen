...
| 时间 | 内容 | 结果 |
|---|---|---|
| 历史迭代 | 完成输入/能力契约、P0/P1 UAF 证据、Claude loop、常驻 SSH-QEMU、原始日志和专家结果路径化 | 已完成（真实 QEMU E2E 待部署环境） |
| 当前 | 工具专家 Semcode 源码证据 adapter、Kernel Expert PoC 源码复核约束 | 进行中 |
...
- crash/lock 工具专家可通过受控 Semcode tools 查询函数、caller/callee、类型和调用链，结果写入结构化 evidence。

### 已知缺口

- P2 已提供 semcode 直接 callee 事件图和工具专家源码证据 adapter；尚无全内核完整性证明、跨回调/间接调用解析或可配置的有界传递闭包。
...
### P2：工具专家源码证据与 Kernel Expert 协同（进行中）

1. crash/lock 专家在 crash 工具分析后，通过受控 Semcode tools 核对关键函数、caller/callee、类型和调用链。
2. 统一输出 `function/file/line/caller/callee/type/source` 结构化 evidence；索引缺失、函数不存在或查询失败必须显式记录。
3. Kernel Expert 优先消费工具专家 evidence，仅对关键因果链补充 Semcode 查询；构造或修改 PoC 前必须完成源码复核。
4. 增加源码 evidence 到 PoC `execution_steps` 的一致性检查，最终报告保留源码依据和复现路径。

验收：真实 vmcore/vmlinux + kernel source + Semcode index 下，crash/lock 专家能返回可追溯源码 evidence，Kernel Expert 能据此完成 PoC 复核；缺少 Semcode 依赖时不得猜测源码结论。
