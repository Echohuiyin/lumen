#!/bin/bash
# Boot ARM64 kernel in QEMU
# Usage: boot_arm64.sh --kernel <path> --initrd <path> [--interactive] [--timeout <seconds>]
...
if [ -z "$KERNEL" ]; then
    ...
    echo "Usage: boot_arm64.sh --kernel <path> --initrd <path>"
    exit 1
fi
...
if ! command -v qemu-system-aarch64 &> /dev/null; then
    ...
    echo "  CentOS/RHEL: yum install qemu-system-aarch64"
    exit 1
fi
...
echo "Machine: virt"
echo "CPU: cortex-a57 ($CPU_COUNT cores)"
echo "Memory: $MEMORY"
...
if [ $INTERACTIVE -eq 1 ]; then
    # Interactive mode: just run QEMU
    eval $QEMU_CMD
else
    # Automated mode: capture output with timeout
