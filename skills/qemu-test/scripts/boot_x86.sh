...
# Usage: boot_x86.sh --kernel <path> --initrd <path> [--interactive] [--timeout <seconds>]
...
if ! command -v qemu-system-x86_64 &> /dev/null; then
    ...
    echo "Please install QEMU for x86:"
    echo "  Ubuntu/Debian: apt install qemu-system-x86"
    echo "  CentOS/RHEL: yum install qemu-kvm"
    exit 1
fi

echo "========================================="
echo "  Booting x86_64 Kernel in QEMU"
echo "========================================="
echo
echo "Kernel: $KERNEL ($(du -h "$KERNEL" | cut -f1))"
echo "Initramfs: $INITRD ($(du -h "$INITRD" | cut -f1))"
echo "Architecture: x86_64"
echo "CPU: QEMU Virtual CPU ($CPU_COUNT cores)"
echo "Memory: $MEMORY"
