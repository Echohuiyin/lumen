...
- **存在 hung task**（大量 D 状态进程、"blocked for more than" 告警）：读取 [`references/reference/scenario_hung_task.md`](references/reference/scenario_hung_task.md)，按其排查流程分析 D 状态进程的阻塞原因
- **存在 soft lockup 或 hard lockup**：读取 [`references/reference/scenario_lockup.md`](references/reference/scenario_lockup.md)，按其排查流程分析 lockup 的根因
