---
name: vmcore-analyzer
description: Linux内核vmcore/coredump分析专家技能。当用户需要分析Linux内核crash dump（vmcore）、排查内核panic原因、调试内核死锁/hung task/softlockup/hardlockup/BUG_ON/空指针等问题时，应使用此skill。
description_zh: Linux内核vmcore崩溃转储分析专家
description_en: Linux kernel vmcore crash dump analysis expert
version: 0.1.0
---

# vmcore-analyzer — Linux 内核 vmcore 分析专家
...
**必须使用 `crash_report_generator.py` 脚本生成报告（Markdown + 命令引用方案）。脚本位置：项目根目录 `scripts/crash_report_generator.py`。**
