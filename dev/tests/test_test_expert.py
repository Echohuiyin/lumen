...
def test_progress_gate_stops_repeated_regressions_with_same_target_failure(tmp_path):
    contract = _contract(tmp_path)
    result = TestResultContract(
        status="failed", code="FAILED_REPRODUCER_REGRESSION",
        attempts=6, missing_frames=["target_frame"], plan=_build_plan(contract),
    )
    previous = [
        {
            "verified_setup": ["SETUP_READY"],
            "best_call_chain_prefix": [],
    ...
    assert updated.status == "blocked"
    assert updated.code == "BLOCKED_PROGRESS_GATE"
    assert updated.progress_kind == "retracted"
    assert updated.no_progress_streak == 2
