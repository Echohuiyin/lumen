...
The deliverable is a bounded maintenance diagnostic. Do not provide instructions for security abuse, persistence, authorization bypass, or arbitrary kernel-state manipulation. A generated userspace C harness may only exercise the documented ABI needed to validate the supplied maintenance regression inside the isolated guest. When `input.txt` explicitly declares an existing operator `.c`, that immutable source is an authorized regression artifact: preserve it byte-for-byte and let Test Expert compile/run it in the isolated guest, even when the upstream syzkaller harness uses privileged setup that a newly generated public-ABI diagnostic cannot reproduce.
For allocator or function-pointer reports, generated tests must use only documented public userspace ABIs: do not construct invalid pointers, alter kernel memory, or invoke undocumented/private ioctls. Do not block an explicit operator `.c` solely because its already-declared harness is privileged or uses syzkaller-private setup; block only when the declared artifact, exact source evidence, or guest capability is actually unavailable.
...
5. Write a diagnostic userspace C test program in the session output directory. It must exercise a documented userspace ABI. When input.txt explicitly declares an existing `.c`, read and reuse that source verbatim; do not rewrite or substitute it. When input.txt explicitly declares a prebuilt `.ko`, it may be loaded as a known binary with exactly one structured `load_module` step; never compile or modify it. In all other cases do not create an in-kernel extension or arbitrary shell workflow.
...
- A generated C test must be ordinary userspace code with declared compiler, source files, link libraries, arguments, and a bounded timeout. An input-declared `.c` is immutable first-hand operator evidence and must be compiled and run by the workflow byte-for-byte; mark it `operator_supplied=true` and declare one `run_binary` step for its output. If an input-declared `.ko` is present, set `prebuilt_module_authorized=true` and include one `load_module` step naming that exact module; never generate a module. If neither an explicit operator artifact nor a documented userspace ABI can exercise the diagnosed path, return `blocked` with the precise missing condition.
...
- For the exact JFS `diFree` error-unwind case, verify from the target commit that `jfs_evict_inode()` requires `!i_nlink && !is_bad_inode()` before calling `diFree()`. A formatter-created FILESYSTEM_I inode may not satisfy that guard; validate the source-defined inode state and filesystem fixture through documented setup APIs. Do not handcraft or alter raw on-disk structures from guessed offsets; if the required state cannot be reached through the documented userspace interface, return a source-backed blocked contract. An `EINVAL` without the required cleanup frames is not a pass.
...
- For this provisioned jfsutils 1.1.15 case, use only the exact guest formatter and bounded image size established by first-hand evidence. Treat the command and size as case-specific setup, not a generic filesystem rule. Validate the formatter's reported inode/xtree metadata before mounting; an empty root is a fixture/layout failure, not a valid precondition. If the target state requires raw metadata alteration rather than a documented userspace operation, report the limitation instead of generating an unsafe fixture.
...
For a declared prebuilt module, `execution_steps` must contain exactly one
`{"type":"load_module","path":"modules/<declared-module-name>.ko"}` before
the userspace `run_binary` step. If the module or operator source is missing,
return `blocked`; do not fall back to a generated trigger.
