...
优先用 semcode MCP 工具定位内核符号，避免全树 Grep：

- `find_function` — 按名找函数/宏定义
- `find_callers` / `find_callees` — 找调用者/被调用者
- `find_type` — 找 struct/typedef
- `find_callchain` — 完整调用链

semcode 找不到时才退回 Grep+Read。Read 只读 semcode 返回的具体行号范围，不要读整个文件。
