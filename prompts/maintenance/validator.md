...
| **可启动内核镜像** | 需要 QEMU 复现验证时提供，必须是 bzImage/Image/zImage，不是 ELF vmlinux | "/path/to/arch/x86/boot/bzImage" |
...
| **需要 QEMU 验证** | 建议：可启动 kernel/Image/bzImage/zImage 路径；不要把 ELF vmlinux 当成可启动内核 |
...
boot_kernel 文件：/data/kernels/5.10.0-60.43.0/arch/arm64/boot/Image
可复现，高并发网络包处理场景触发。
```
...
- boot_kernel：/data/kernels/5.10.0-60.43.0/arch/arm64/boot/Image
- 复现条件：高并发网络包处理场景
...
4. 如果需要 QEMU 验证，提供可启动 kernel/Image/bzImage/zImage 路径
5. 死锁发生的场景或触发条件
...
boot_kernel 文件：/data/kernels/5.10.0/arch/x86/boot/bzImage
...
- 根据问题类型，提示用户需要提供的特定信息（如 vmcore 问题需要 vmcore 和 vmlinux 路径；QEMU 验证需要可启动 kernel/Image/bzImage/zImage 路径）
- 明确区分 vmlinux 和 boot kernel：vmlinux 用于 crash 调试符号，不可直接作为 x86_64 QEMU `-kernel` 启动镜像
